...
while [ "$i" -lt "$n" ]; do
  ...
  echo "  [killed] $sname"
  while IFS= read -r browser_port; do
    sw_browser_release_port "$PROJECT_ID" "$browser_port"
  done < <(printf '%s' "$CONFIG_JSON" | jq -r --arg sid "$sid" '[.browser?, .browsers[]?][] | select(. != null and .session_id == $sid) | .port')
  KILLED=$((KILLED + 1))
done
