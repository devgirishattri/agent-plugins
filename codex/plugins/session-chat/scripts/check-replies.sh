...
# <id> is a message id this pane previously sent. The reply transport generates
# [re:<id>] automatically; this reporter never infers task liveness.
# Usage: check-replies.sh [--pending] [--since MINUTES]
#   --pending        only show sent messages without a correlated reply
...
if [ "$ROWS" -eq 0 ]; then
  ...
  echo "—"
  echo "${ROWS} message(s) shown, ${PENDING} unconfirmed. Correlation is generated by \$session-chat:reply; this is not task-liveness status."
fi
exit 0
