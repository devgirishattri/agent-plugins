...
After the scripts pass, complete step 10 with a fresh subagent. Give the reviewer only the target path, the resolved absolute `PLUGIN_ROOT`, and the independent worker procedure from `skills/docs-review/SKILL.md`; explicitly tell it not to delegate again. Apply no fixes during that review pass. If it reports issues, present them to the user or make fixes in a separate editing pass and then request another fresh review.
