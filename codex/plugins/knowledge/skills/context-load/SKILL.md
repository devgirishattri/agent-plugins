...
Internalize the loaded context, especially what was done, files changed, decisions, open issues, and where the prior session left off. Treat it as temporary working state, not durable truth: use it to continue the work, and if it has stabilized, suggest `$knowledge:promote`; if it is obsolete, suggest separately confirmed cleanup with `$knowledge:context-remove`. Summarize what was loaded. If a staleness WARNING appears at the end of the output (snapshot 7 or more days old, configurable via `SESSION_CONTEXT_STALE_DAYS`), surface it and suggest regenerating with `$knowledge:context-generate`, promoting it, or removing it explicitly. If no snapshot is found, suggest `$knowledge:context-list`.
