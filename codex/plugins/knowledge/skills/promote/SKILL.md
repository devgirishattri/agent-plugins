...
prior step (a plain read-only Bash command, or the file-reading, file-editing, and search tools),
...
same rule every `$knowledge:context-*` command follows). If a step below needs it and
...
user as text. Do not use the file-editing tool, do not invoke `docs-write.sh` (that
...
Ask the user (or read the user's arguments) which of these this run promotes:
...
1. **Re-read with the file-reading tool** `<STORE_PATH>/MEMORY.md` right now (fresh, not the step-4 copy).
2. **Write with the file-editing tool** the final target content and the final MEMORY.md content to two
...
distinct question from step 6's destination approval. Ask the user, presenting **"No, keep the source (Recommended)" FIRST as the default**, then
...
2. **Write with the file-editing tool** the staged post-removal MEMORY.md content (the index with the
   source's membership row removed) to a scratch file.
3. Invoke exactly one literal Bash segment:
   ```
   bash "<PLUGIN_ROOT>/scripts/memory-write.sh" retire \
