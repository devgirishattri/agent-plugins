...
1. If `$ARGUMENTS` is empty, tell the user: `Usage: $knowledge:context-load <snapshot-name>`.
...
6. If a staleness WARNING appears at the end of the output, surface it to the user and suggest regenerating with `$knowledge:context-generate <name>` — treat the loaded content as potentially out of date.
7. If no snapshot is found, suggest `$knowledge:context-list`.
