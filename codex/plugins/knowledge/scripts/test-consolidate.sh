...
# PROVIDER (exercised only once the Claude mirror's memory-remember.sh lands)
# ===========================================================================
...
stage_candidate "$f_cp" "sess-cp-codex" "normal" "Cross Provider Consolidation Item" "captured under one provider, consolidated after being listed from the other" project
...
if [ -f "$CLAUDE_REMEMBER" ]; then
  ...
  assert_rc "cross_provider_consolidate_claude_list_exit0" 0 "$rc"
  assert_contains "cross_provider_consolidate_claude_sees_codex_capture" "$out_claude_list" "$cpkey"
else
  echo "  SKIP  cross_provider_consolidate -- plugins/knowledge/scripts/memory-remember.sh not present yet; this fixture will be exercised at the E/F gates once the Claude mirror lands."
fi
