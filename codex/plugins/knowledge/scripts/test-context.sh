...
assert_contains "$TMP/session-chat.capture" '/knowledge:context-load alpha'
assert_contains "$TMP/session-chat.capture" '$knowledge:context-load alpha'
...
# Scoped to the absorbed context-* surface (plus the knowledge overview, which
# documents the same contract): plugins/knowledge is a multi-module plugin
# now, and its other commands/skills (docs-*, and later doctor/lint/...) have
# nothing to do with SESSION_CONTEXT_HOME.
