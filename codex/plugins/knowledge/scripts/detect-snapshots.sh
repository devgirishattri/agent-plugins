#!/usr/bin/env bash
# detect-snapshots.sh — SessionStart hook for knowledge's context-store
# surface (Codex; absorbed from session-context 0.7.8).
...
emit_system_message() {
  local message="$1"
  local suffix=" [truncated by knowledge to fit Codex additionalContext limit]"
  local max_len=10000
...
emit_system_message "knowledge: ${count} context snapshot(s) available for this project: ${names}. Run \$knowledge:context-load <name> to resume prior work, or \$knowledge:context-list for details."
exit 0
