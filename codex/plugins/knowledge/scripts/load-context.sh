...
if [ -n "$mtime" ]; then
  ...
  if [ "$age_days" -ge "$STALE_DAYS" ]; then
    echo ""
    echo "WARNING: Snapshot '$PROJECT_NAME' is ${age_days} day(s) old (stale threshold: ${STALE_DAYS} days). It may no longer reflect the project — consider regenerating it with \$knowledge:context-generate ${PROJECT_NAME}."
  fi
fi
