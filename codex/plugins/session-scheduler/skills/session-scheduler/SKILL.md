...
- Scripts require `SESSION_SCHEDULER_HOME` and fail closed without it rather than guessing a cwd/tmp location; the fix is to relaunch the pane/session with the correct environment. Direct human script use may set the variable in the parent shell before invoking a script, but generated agent instructions never combine environment setup with helper execution.
