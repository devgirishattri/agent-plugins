...
- `--context NAME` attaches an existing canonical context snapshot (lowercase snake_case matching `^[a-z0-9]+(_[a-z0-9]+)*$`). `--context auto` creates a private immutable task handoff named `auto_handoff_<random-hex>` from the approved prompt and current ledger state, then attaches it; the 32-character lowercase-hex nonce comes only from OS randomness and never contains task ids or dates.
