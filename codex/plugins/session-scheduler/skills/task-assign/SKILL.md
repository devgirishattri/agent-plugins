...
- `--context NAME` — attach the session-context snapshot under `SESSION_CONTEXT_HOME` (exported automatically to `<git-root>/tmp/contexts`, or pwd when not in a git repo, unless already set); the prompt tells the executor to `$session-context:context-load NAME` first.
