...
5. Independently of reviewer routing, an eligible assigner receives a durable file-backed lifecycle acknowledgement, queued to the assigner's inbox when busy. The ledger remains authoritative, and `meta.last_ack` records the acknowledgement outcome without colliding with reviewer-dispatch metadata. If assigner acknowledgement delivery fails, the task stays in `review`; never rerun the transition to repair it.
6. The reviewer approves with `$session-scheduler:task-done <task-id> <note>` or rejects with `$session-scheduler:task-block <task-id> <reason>`. Review state remains recorded if dispatch needs retrying.

A dispatch-only retry reuses the note from the latest history event `review`. The CLI note remains syntactically required but is ignored on retry; the script prints `note: <original> (original review note reused on retry)`.
