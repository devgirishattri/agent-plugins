...
run_sender() {
  TMUX="$TMUX_ENV" TMUX_PANE="$SENDER" CODEX_HOME="$TEST_HOME" SESSION_CHAT_PLUGIN_ROOT="$SESSION_CHAT_ROOT" "$@"
}

run_recipient() {
  TMUX="$TMUX_ENV" TMUX_PANE="$RECIPIENT" CODEX_HOME="$TEST_HOME" SESSION_CHAT_PLUGIN_ROOT="$SESSION_CHAT_ROOT" "$@"
}

SESSION_CHAT_ROOT="${SESSION_CHAT_PLUGIN_ROOT:-$SCRIPT_DIR/../../session-chat}"
[ -d "$SESSION_CHAT_ROOT" ] || SESSION_CHAT_ROOT="$HOME/.codex/plugins/cache/girishattri-codex-plugins/session-chat/0.11.0"
