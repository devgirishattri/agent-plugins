...
[ -d "$SESSION_CHAT_ROOT" ] && ok || fail "session-chat test dependency missing: $SESSION_CHAT_ROOT"
...
[ "$(path_mode "$LEGACY_SCHEDULER")" = "700" ] && ok || fail "legacy scheduler root was not migrated to mode 700"
[ "$(path_mode "$LEGACY_SCHEDULER/tasks")" = "700" ] && ok || fail "legacy tasks directory was not migrated to mode 700"
[ "$(path_mode "$LEGACY_SCHEDULER/prompts")" = "700" ] && ok || fail "legacy prompts directory was not migrated to mode 700"
[ "$(path_mode "$LEGACY_SCHEDULER/tasks/legacy.json")" = "600" ] && ok || fail "legacy task record was not migrated to mode 600"
[ "$(path_mode "$LEGACY_SCHEDULER/prompts/legacy.md")" = "600" ] && ok || fail "legacy prompt was not migrated to mode 600"
...
grep 'Refusing unsafe scheduler root' "$TEST_HOME/unsafe-scheduler.err" >/dev/null \
  && ok || fail "scheduler symlink-root rejection did not explain the unsafe root"
[ ! -e "$UNSAFE_SCHEDULER_TARGET/tasks" ] && ok || fail "scheduler wrote through the rejected symlink root"
...
grep 'scheduler tree containing a symlink' "$TEST_HOME/unsafe-task-tree.err" >/dev/null \
  && ok || fail "nested task-record symlink rejection did not explain the unsafe tree"
...
for race_pid in $scheduler_race_pids; do
  wait "$race_pid" && ok || fail "concurrent scheduler root initialization failed"
done
[ "$(path_mode "$RACE_SCHEDULER")" = "700" ] && ok || fail "concurrently initialized scheduler root is not mode 700"
[ "$(path_mode "$RACE_SCHEDULER/tasks")" = "700" ] && ok || fail "concurrently initialized tasks directory is not mode 700"
[ "$(path_mode "$RACE_SCHEDULER/prompts")" = "700" ] && ok || fail "concurrently initialized prompts directory is not mode 700"
...
grep 'unknown IANA timezone' "$TEST_HOME/invalid-timezone.out" >/dev/null \
  && ok || fail "invalid timezone rejection lacked guidance"

created=$(run_sender bash "$SCRIPT_DIR/task-new.sh" "Smoke task" --meta area=test)
echo "$created" | grep 'Created task' >/dev/null && ok || fail "task-new did not create"
TASK_ID=$(printf '%s\n' "$created" | awk '/^Created task/{print $3}' | sed 's/:$//')
[ -f "$TEST_HOME/scheduler/tasks/$TASK_ID.json" ] && ok || fail "task file missing"
[ "$(jq -r '.created_at' "$TEST_HOME/scheduler/tasks/$TASK_ID.json")" != "" ] \
  && ok || fail "task-new did not record created_at"
...
[[ "$(jq -r '.created_at' "$TEST_HOME/scheduler/tasks/$TASK_ID.json")" == *"$expected_offset" ]] \
  && ok || fail "task-new did not store created_at in the configured timezone"
[ "$(path_mode "$TEST_HOME/scheduler")" = "700" ] && ok || fail "scheduler root is not mode 700"
[ "$(path_mode "$TEST_HOME/scheduler/tasks")" = "700" ] && ok || fail "tasks directory is not mode 700"
[ "$(path_mode "$TEST_HOME/scheduler/prompts")" = "700" ] && ok || fail "prompts directory is not mode 700"
[ "$(path_mode "$TEST_HOME/scheduler/tasks/$TASK_ID.json")" = "600" ] && ok || fail "new task record is not mode 600"
run_sender bash "$SCRIPT_DIR/tasks-clean.sh" --older-than 30d > "$TEST_HOME/clean-fresh.txt"
[ -f "$TEST_HOME/scheduler/tasks/$TASK_ID.json" ] && ok || fail "tasks-clean selected a fresh offset-bearing task"

run_sender bash "$SCRIPT_DIR/task-assign.sh" scheduler-executor "$TASK_ID" "Do the smoke task"
[ "$(path_mode "$TEST_HOME/scheduler/prompts/$TASK_ID.md")" = "600" ] && ok || fail "assignment prompt is not mode 600"
...
[ -n "$ASSIGN_MESSAGE_FILE" ] && [ -f "$ASSIGN_MESSAGE_FILE" ] && ok || fail "assignment dispatch notification did not reach recipient"
grep "$TASK_ID" "$ASSIGN_MESSAGE_FILE" >/dev/null && ok || fail "assignment dispatch body omitted the task id"
...
grep -F "Shared scheduler home (provenance): $EXPECTED_SCHEDULER_HOME" "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing exact canonical scheduler home provenance"
grep -F 'inherited' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing inherited-environment guidance"
grep -F 'relaunch' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing relaunch guidance"
grep -F 'Transport contract:' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing nested-transport contract"
grep -F 'on the first attempt' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing first-attempt escalation guidance"
...
grep -F '$session-scheduler:task-done' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing Codex completion command"
grep -F '/session-scheduler:task-done' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing Claude completion command"
grep -F '$session-scheduler:task-review' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing Codex review command"
grep -F '/session-scheduler:task-review' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing Claude review command"
grep -F '$session-scheduler:task-block' "$ASSIGN_MESSAGE_FILE" >/dev/null \
  && ok || fail "assignment packet missing Codex block command"
grep -F '/session-scheduler:task-block' "$ASSIGN_MESSAGE_FILE" >/dev/null \
...
grep 'Summary' "$TEST_HOME/clean-apply.txt" >/dev/null && ok || fail "clean apply missing summary"
...
[ -f "$OUTSIDE_PROMPT" ] && ok || fail "tasks-clean deleted an outside prompt_file path"
[ ! -f "$TEST_HOME/scheduler/prompts/$LEGIT_ID.md" ] && ok || fail "tasks-clean did not delete a legitimate prompt file"
[ ! -f "$TEST_HOME/scheduler/tasks/$MALICIOUS_ID.json" ] && ok || fail "tasks-clean did not delete malicious task record"
[ ! -f "$TEST_HOME/scheduler/tasks/$LEGIT_ID.json" ] && ok || fail "tasks-clean did not delete legit task record"

run_sender bash "$SCRIPT_DIR/scheduler-doctor.sh" | grep 'session-chat version' >/dev/null && ok || fail "doctor missing session-chat version"
run_sender bash "$SCRIPT_DIR/scheduler-doctor.sh" | grep 'date math: OK' >/dev/null && ok || fail "doctor date math check failed"
...
grep 'Illegal status transition' "$TEST_HOME/illegal.txt" >/dev/null && ok || fail "illegal transition error message missing"
run_sender bash "$SCRIPT_DIR/task-status.sh" "$ILLEGAL_ID" | grep 'created' >/dev/null && ok || fail "status changed despite rejected transition"
...
printf '%s\n' "$forced_note" | grep 'forced' >/dev/null && ok || fail "forced transition did not record 'forced' in note"
...
[ -n "$duration" ] && ok || fail "duration_seconds not recorded on done"
...
grep 'unmet dependencies' "$TEST_HOME/gated.txt" >/dev/null && ok || fail "unmet dependency error missing"
grep "$DEP_ID" "$TEST_HOME/gated.txt" >/dev/null && ok || fail "unmet dependency error does not name the dep"
...
run_sender bash "$SCRIPT_DIR/task-assign.sh" scheduler-executor "$GATED_ID" "Gated work" && ok || fail "assign failed after dependency done"
...
grep 'does not exist' "$TEST_HOME/baddep.txt" >/dev/null && ok || fail "missing-dependency error missing"
...
[ -n "$eta_at" ] && ok || fail "eta_at not stored"
...
run_sender bash "$SCRIPT_DIR/task-status.sh" | grep "$ETA_ID" | grep 'OVERDUE' >/dev/null && ok || fail "OVERDUE flag missing"
...
[ "$(jq -r '.status' "$TEST_HOME/scheduler/tasks/$BLK_ETA_ID.json")" = "blocked" ] \
  && ok || fail "blocked-eta task not in blocked state"
...
grep 'Stage: execute' "$TEST_HOME/board.txt" >/dev/null && ok || fail "board missing stage group"
grep "$ETA_ID" "$TEST_HOME/board.txt" >/dev/null && ok || fail "board missing task row"
grep 'OVERDUE' "$TEST_HOME/board.txt" >/dev/null && ok || fail "board missing OVERDUE flag"
grep -E '[0-9]+ active:' "$TEST_HOME/board.txt" >/dev/null && ok || fail "board missing totals summary"

# --- task-status --by-stage groups output ---
run_sender bash "$SCRIPT_DIR/task-status.sh" --by-stage > "$TEST_HOME/bystage.txt"
grep 'Stage: execute' "$TEST_HOME/bystage.txt" >/dev/null && ok || fail "by-stage missing stage group"
grep "$ETA_ID" "$TEST_HOME/bystage.txt" >/dev/null && ok || fail "by-stage missing task row"
...
' "$TEST_HOME/bystage.txt" && ok || fail "by-stage row lost its flags column or TSV shape"
...
grep -F "Shared context home (provenance): $EXPECTED_CONTEXT_HOME" "$TEST_HOME/scheduler/prompts/$CTX_ID.md" >/dev/null \
  && ok || fail "context assignment packet missing exact canonical context home provenance"
...
[ -n "$REVIEW_MESSAGE_FILE" ] && [ -f "$REVIEW_MESSAGE_FILE" ] && ok || fail "review dispatch notification did not reach reviewer"
grep "$ROUTED_ID" "$REVIEW_MESSAGE_FILE" >/dev/null && ok || fail "review dispatch body omitted the task id"
jq -e '
...
' "$ROUTED_FILE" >/dev/null && ok || fail "canonical review dispatch metadata missing or legacy alias retained"
ROUTED_REVIEW_PACKET="$TEST_HOME/scheduler/prompts/${ROUTED_ID}-review.md"
[ "$(path_mode "$ROUTED_REVIEW_PACKET")" = "600" ] && ok || fail "review prompt is not mode 600"
...
grep -F 'relaunch' "$TEST_HOME/routed-review-instructions.txt" >/dev/null \
  && ok || fail "review packet missing relaunch guidance"
grep -F 'Transport contract:' "$TEST_HOME/routed-review-instructions.txt" >/dev/null \
  && ok || fail "review packet missing nested-transport contract"
grep -F 'on the first attempt' "$TEST_HOME/routed-review-instructions.txt" >/dev/null \
  && ok || fail "review packet missing first-attempt escalation guidance"
...
grep -F '$session-scheduler:task-done' "$TEST_HOME/routed-review-instructions.txt" >/dev/null \
  && ok || fail "review packet missing Codex approval command"
grep -F '/session-scheduler:task-done' "$TEST_HOME/routed-review-instructions.txt" >/dev/null \
  && ok || fail "review packet missing Claude approval command"
...
grep 'Not re-dispatching' "$TEST_HOME/legacy-review.out" >/dev/null \
  && ok || fail "legacy success timestamp did not prevent duplicate review dispatch"
[ ! -f "$TEST_HOME/scheduler/prompts/${LEGACY_REVIEW_ID}-review.md" ] \
  && ok || fail "legacy success metadata still caused a duplicate review packet"
jq -e '
...
' "$LEGACY_REVIEW_FILE" >/dev/null && ok || fail "legacy review metadata was not canonically migrated"
...
grep 'Workflow: release-42' "$TEST_HOME/byworkflow.txt" >/dev/null && ok || fail "workflow group disappeared after its task completed"
grep "$ROUTED_ID" "$TEST_HOME/byworkflow.txt" | grep 'done' >/dev/null && ok || fail "by-workflow omitted its completed task"
...
grep 'failed safety checks' "$TRAVERSAL_PACKET" >/dev/null && ok || fail "unsafe traversal prompt_file was not explicitly rejected"
...
grep 'scheduler tree containing a symlink' "$TEST_HOME/prompt-symlink.err" >/dev/null \
  && ok || fail "nested prompt symlink rejection did not explain the unsafe tree"
...
run_sender bash "$SCRIPT_DIR/task-status.sh" "$SYMLINK_REVIEW_ID" | grep 'assigned' >/dev/null \
  && ok || fail "rejected prompt symlink changed task state"
...
grep 'session-chat >= 0.13.0 is required' "$TEST_HOME/low-chat.txt" >/dev/null && ok || fail "minimum-version error missing"
run_sender bash "$SCRIPT_DIR/task-status.sh" "$LOW_ID" | grep 'created' >/dev/null && ok || fail "old transport failure changed ledger"
...
grep -F 'Do NOT rerun task-done' "$TEST_HOME/partial-done.err" >/dev/null \
  && ok || fail "task-done notification warning omitted replay prohibition"
...
grep -F 'Do NOT rerun task-block' "$TEST_HOME/partial-block.err" >/dev/null \
  && ok || fail "task-block notification warning omitted replay prohibition"
...
[ -n "$(jq -r '.meta.review_dispatched_at // empty' "$RETRY_REVIEW_FILE")" ] \
  && ok || fail "first review cycle did not record dispatch success"
...
' "$RETRY_REVIEW_FILE" >/dev/null && ok || fail "reassignment retained canonical or legacy review dispatch metadata"
...
[ "$(jq -r '.status' "$RETRY_REVIEW_FILE")" = "review" ] && ok || fail "failed reviewer dispatch did not leave task in review"
[ -z "$(jq -r '.meta.review_dispatched_at // empty' "$RETRY_REVIEW_FILE")" ] \
  && ok || fail "failed reviewer dispatch recorded a success timestamp"
[ "$(jq '[.history[] | select(.event == "review")] | length' "$RETRY_REVIEW_FILE")" -eq 2 ] \
  && ok || fail "second review cycle did not record exactly one new review event"
[ "$(jq -r '.meta.review_dispatch_attempts // 0' "$RETRY_REVIEW_FILE")" -eq 1 ] \
  && ok || fail "failed reviewer dispatch attempt was not recorded"
jq -e '
  .meta.review_dispatch_status == "failed"
  and (.meta.review_dispatch_attempt_at // "") != ""
  and (.meta.review_dispatch_error | startswith("session-chat dispatch failed"))
  and ([keys[] | select(startswith("review_"))] | length) == 0
' "$RETRY_REVIEW_FILE" >/dev/null && ok || fail "failed review dispatch metadata is not canonical"
...
grep 'Retried review dispatch' "$TEST_HOME/retry-review.txt" >/dev/null \
  && ok || fail "task-review did not report a dispatch-only retry"
[ "$(jq -r '.status' "$RETRY_REVIEW_FILE")" = "review" ] && ok || fail "review retry changed task status"
[ "$(jq '[.history[] | select(.event == "review")] | length' "$RETRY_REVIEW_FILE")" -eq 2 ] \
  && ok || fail "review retry created a duplicate review history event"
[ "$(jq -r '[.history[] | select(.event == "review")][-1].note' "$RETRY_REVIEW_FILE")" = "original review note" ] \
  && ok || fail "review retry overwrote the original review note"
[ -n "$(jq -r '.meta.review_dispatched_at // empty' "$RETRY_REVIEW_FILE")" ] \
  && ok || fail "successful review retry did not record its dispatch timestamp"
[ "$(jq -r '.meta.review_dispatch_attempts // 0' "$RETRY_REVIEW_FILE")" -eq 2 ] \
  && ok || fail "review dispatch retry attempt count is wrong"
[ "$(jq -r '.meta.review_dispatch_error // empty' "$RETRY_REVIEW_FILE")" = "" ] \
  && ok || fail "successful review retry did not clear dispatch error metadata"
jq -e '
  (.meta.review_dispatch_status == "delivered" or .meta.review_dispatch_status == "queued")
  and ([keys[] | select(startswith("review_"))] | length) == 0
' "$RETRY_REVIEW_FILE" >/dev/null && ok || fail "successful review retry did not retain canonical status only"
...
[ ! -f "$TEST_HOME/scheduler/prompts/$RB_ID.md" ] && ok || fail "orphaned prompt file left after dispatch failure"
run_sender bash "$SCRIPT_DIR/task-status.sh" "$RB_ID" | grep 'created' >/dev/null && ok || fail "ledger updated despite dispatch failure"
...
while IFS= read -r persisted; do
  [ "$(path_mode "$persisted")" = "600" ] && ok || fail "scheduler data file is not mode 600: $persisted"
...
for task_doc in task-assign task-review task-done task-block; do
  for doc in \
    ...
    grep -F 'one literal Bash segment' "$doc" >/dev/null \
      && ok || fail "transport doc missing literal Bash segment contract: $doc"
    grep -F 'use --force to repair' "$doc" >/dev/null \
      && ok || fail "transport doc missing force-repair prohibition: $doc"
  done
done
...
grep -F 'Transport contract' "$PLUGIN_DOC_ROOT/skills/session-scheduler/SKILL.md" >/dev/null \
  && ok || fail "umbrella skill missing transport contract"
grep -F 'on the first attempt' "$PLUGIN_DOC_ROOT/skills/session-scheduler/SKILL.md" >/dev/null \
  && ok || fail "umbrella skill missing first-attempt escalation guidance"

echo "session-scheduler smoke tests: $ASSERTIONS passed, 0 failed"
