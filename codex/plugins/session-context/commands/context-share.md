...
6. Report: `Shared session context '<snapshot-name>' with <session>. The notification does not copy the file; they can load it with $session-context:context-load <snapshot-name> only when both panes inherited the same SESSION_CONTEXT_HOME. Relaunch a mismatched pane with the correct environment.`
