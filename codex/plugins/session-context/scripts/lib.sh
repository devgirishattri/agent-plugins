...
resolve_pane() {
  ...
  if [ -z "$result" ]; then
    echo "ERROR: No pane named '$label'. Run /panes or \$session-chat:panes to see available." >&2
    return 1
  fi
  echo "$result"
}
...
send_message() {
  ...
  my_name=$(get_my_name)
  if [ -z "$my_name" ]; then
    echo "ERROR: This pane has no name. Run /whoami <name> or \$session-chat:whoami <name> first." >&2
    return 1
  fi
  local target_pane
