...
2. Delegate the verification to the **doc-reviewer** subagent via the Agent tool (subagent_type `knowledge:doc-reviewer`). It independently checks that every referenced file path, function, table, endpoint, env var, and cross-link in the target docs actually exists in the codebase, and runs the plugin's validation scripts:
