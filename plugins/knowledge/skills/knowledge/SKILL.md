...
**Docs — see the absorbed `creating-docs` skill for the full process:**
...
**Context — absorbed from session-context 0.7.8; see the absorbed
`session-context` skill for the full lifecycle:**
...
context directory). See the absorbed `session-context` skill for the full
...
- `skills/creating-docs/SKILL.md` — the full structured docs-authoring
  process (reference-based notation, templates, validation scripts).
- `skills/session-context/SKILL.md` — the full context-snapshot lifecycle,
...
`knowledge >= 0.1.0` — the dependency equivalence any consumer (e.g. a
`session-workspace`-style plugin) should treat as met.
...
2. **Confirm parity.** Run `/knowledge:doctor` (Claude) or `$knowledge:doctor`
   (Codex) with both plugins still enabled and review its findings.
...
*enabled* at once, `doctor`'s duplicate-enabled-plugin detector (a static
scan of `enabledPlugins` in `$HOME/.claude/settings.json` and the repo's
`.claude/settings.json` / `.claude/settings.local.json`) reports exactly
(verbatim from `doctor.sh`, `section_duplicate_plugins`, reproduced against a
synthetic fixture during Phase G):
...
Disabling `session-context` (step 3) clears the WARN on the next doctor run.
...
- Neither store lives inside `plugins/session-context/`,
  ...
  installed DIRECTORY never touches durable data, which always lives
  outside every plugin's own tree.
