#!/usr/bin/env bash
# request-capture.sh — knowledge 0.3 autonomous-capture Stop hook (CLAUDE tree;
# opt-in). When the gate is ON and this is not a re-entrant Stop, it asks the
# AGENT (via the blocking Stop shape) to run ONE bounded capture pass over its
# own active context and route any candidates through memory-auto-capture.sh.
...
# memory-remember.sh (.inbox only). /consolidate stays the persist gate.
#
...
#                stop_hook_active is true (loop guard), otherwise emits a Claude
