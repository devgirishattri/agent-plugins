...
# Knowledge context-store helpers.
  ...
  find -P "$lock_dir" -mindepth 1 -print0 2>/dev/null | while IFS= read -r -d '' path; do
    ...
    case "$relative" in
      pid)
        ...
        ;;
      .reclaim)
        # A contender creates this empty owner-only directory before moving a
        # stale generation. Its inode is the generation-specific claim: other
        # reclaimers wait, and the claimant must revalidate lock+PID identities
        # after winning it before touching the reusable lock pathname.
...
_context_validate_tree() {
  ...
  # user running /context-list beside an active writer could see it. A store
  # holding even one snapshot did not reproduce it, so re-testing this needs an
  # EMPTY store.
  #
...
_context_lock_generation_token() {
  # Return the identity of one exact reusable-path generation. The PID content
  # alone is insufficient: after an owner releases, another writer may reuse
  # .knowledge-context.lock before a waiter acts on the old PID (the ABA race).
...
_context_quarantine_lock_generation() {
  ...
  # mismatch means pathname turnover occurred, so the new generation is left
  # untouched and the caller retries.
  #
  # mode=dead  — reclaim only if the recorded PID is still dead.
  # mode=owner — release the generation held by this process.
...
session_chat_root() {
  # A candidate only counts when its send-message.sh exists and is readable;
  # a stray directory (or a newest-version entry whose files are missing)
  # must fall through to the next candidate, never win by name alone.
  _sc_candidate_ok() { [ -d "$1" ] && [ -r "$1/scripts/send-message.sh" ] && [ -f "$1/scripts/send-message.sh" ]; }
  if [ -n "${SESSION_CHAT_ROOT_OVERRIDE:-}" ]; then
    # Test/operator override is authoritative even when it points nowhere:
    # a bad override means "no session-chat", not "search elsewhere".
    printf '%s\n' "$SESSION_CHAT_ROOT_OVERRIDE"
    return 0
  fi
  local cache="$HOME/.claude/plugins/cache/girishattri-plugins/session-chat" versioned
  if [ -d "$cache" ]; then
    while IFS= read -r versioned; do
      [ -n "$versioned" ] || continue
      if _sc_candidate_ok "$cache/$versioned"; then
        printf '%s/%s\n' "$cache" "$versioned"
        return 0
      fi
    done < <(ls -1 "$cache" 2>/dev/null | sort -rV)
  fi
  local here
  here=$(cd "$(dirname "${BASH_SOURCE[0]}")/../.." && pwd)
  if _sc_candidate_ok "$here/session-chat"; then
