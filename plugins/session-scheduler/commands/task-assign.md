...
argument-hint: <pane> <id> [--eta MINUTES] [--stage NAME] [--context NAME] [--reviewer PANE] [--workflow ID] [--force] <prompt>
...
- `--context auto` — instead of requiring a pre-existing snapshot, generate a private, **immutable** handoff (`auto-<id>.md`, chmod 400) derived from the approved prompt + ledger state, attach it, and record `meta.context = auto-<id>`. It is removed automatically if the dispatch rolls back. No live-session summarization — safe to generate from the assignment itself.
