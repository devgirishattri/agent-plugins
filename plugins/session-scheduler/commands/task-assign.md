...
- `--context NAME` — attach a session-context snapshot (`$SESSION_CONTEXT_HOME/NAME.md`). Errors if the snapshot is missing. The generated prompt tells the executor to run `/knowledge:context-load NAME` first (with the absolute context home embedded as provenance), and `meta.context` is recorded on the task.
- `--context auto` — instead of requiring a pre-existing snapshot, generate a private, uniquely named owner-read-only handoff (`auto-<id>-<random>.md`, chmod 400) derived from the approved prompt + ledger state, attach it, and record `meta.context = auto-<id>-<random>` (the exact name is printed in the command output). It is removed automatically if the dispatch rolls back. No live-session summarization — safe to generate from the assignment itself.
