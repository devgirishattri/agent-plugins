...
- `--context NAME` — attach a context snapshot (`$SESSION_CONTEXT_HOME/NAME.md`). `NAME` must be canonical `snake_case` (`^[a-z0-9]+(_[a-z0-9]+)*$`) — the knowledge context store's own contract, so no hyphens, uppercase, leading/trailing underscores, or doubled underscores; a non-canonical name is rejected before any side effect. Errors if the snapshot is missing. The generated prompt tells the executor to run `/knowledge:context-load NAME` first (with the absolute context home embedded as provenance), and `meta.context` is recorded on the task.
- `--context auto` — instead of requiring a pre-existing snapshot, generate a private, uniquely named owner-read-only handoff (`auto_handoff_<random-hex>.md`, chmod 400) derived from the approved prompt + ledger state, attach it, and record `meta.context = auto_handoff_<random-hex>` (the exact name is printed in the command output). The stem is canonical `snake_case` and **date-free**: a fixed semantic prefix plus a 32-hex-digit nonce from OS randomness — never the task id, a timestamp, or anything derived from them (dates belong in metadata, not filenames). The task association lives in the handoff body and `meta.context`. It is removed automatically if the dispatch rolls back. No live-session summarization — safe to generate from the assignment itself.
