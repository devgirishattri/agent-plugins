...
- `--context NAME` — attach a context snapshot (`$SESSION_CONTEXT_HOME/NAME.md`). Errors if the snapshot is missing. The generated prompt tells the executor to run `/knowledge:context-load NAME` first (with the absolute context home embedded as provenance), and `meta.context` is recorded on the task.
