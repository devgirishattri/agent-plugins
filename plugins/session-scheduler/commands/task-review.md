...
Lead with the result; add text only for errors or the follow-ups below. The note is required — typically a commit SHA or a one-line summary of what to audit.
...
- The assigner gets a one-line "ready for REVIEW" ack via the same durable delivery ladder as `/task-done`/`/task-block`: file-backed dispatch first (queued to the assigner's inbox when busy), inline `/send` as a last-resort fallback. `meta.last_ack` records the outcome. Only sent the first time the task moves into `review` — a retry (below) never resends it. On total failure this is independent of reviewer routing: a WARN is emitted but reviewer routing still proceeds and the task stays in `review` — never rerun the transition to repair the notification.
...
- Retry rule: re-running `/task-review` after a dispatch failure is a dispatch-only retry, legal only while the task is in `review` with no successful reviewer-dispatch timestamp recorded — never duplicate a delivered review packet (the script refuses), and never use --force to repair a notification. A retry re-sends the **original** review note (the one recorded in history when the task entered review, e.g. the commit SHA); the note typed on the retry is syntactically required but ignored, and the output says `(original review note reused on retry)`.
