...
3. Report the result:
   - "Sent to …" / "Dispatched task to …" → confirm, and note the reply is correlated (the sender's `/check-replies` will mark id `<message-id>` answered).
   ...
   - If "did not land within Xms", the recipient is likely busy; retry once after a short pause.
